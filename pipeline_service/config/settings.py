...
class Settings(BaseSettings):
    ...
    # Trellis settings
    trellis_model_id: str = Field(default="Stable-X/trellis-vggt-v0-2", env="TRELLIS_MODEL_ID")
    trellis_sparse_structure_steps: int = Field(default=30, env="TRELLIS_SPARSE_STRUCTURE_STEPS")
    trellis_sparse_structure_cfg_strength: float = Field(default=7.5, env="TRELLIS_SPARSE_STRUCTURE_CFG_STRENGTH")
    trellis_slat_steps: int = Field(default=25, env="TRELLIS_SLAT_STEPS")
    trellis_slat_cfg_strength: float = Field(default=3, env="TRELLIS_SLAT_CFG_STRENGTH")
    ...
    qwen_edit_prompt_path: Path = Field(default=config_dir.joinpath("prompt_embedding.safetensors"), env="QWEN_EDIT_PROMPT_PATH")
