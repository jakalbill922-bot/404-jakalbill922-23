...
kSqrt02_07 = torch.sqrt(torch.tensor(2.0 / 7.0, dtype=_dtype, device=_device))
...
kSqrt07_15 = torch.sqrt(torch.tensor(7.0 / 10.0, dtype=_dtype, device=_device))
...
def get_sh1_torch(R):
    """
    Compute the SH degree-1 rotation matrix from a 3x3 rotation matrix.
    Args:
        R (torch.Tensor): Rotation matrix (3, 3).
    Returns:
        torch.Tensor: SH degree-1 rotation matrix (3, 3).
    """
    return torch.roll(torch.roll(R, shifts=(-1,), dims=(-1)), shifts=(-1,), dims=(-2))

def get_sh2_torch(sh1):
    """
    Compute the SH degree-2 rotation matrix.
    Args:
        sh1 (torch.Tensor): SH degree-1 rotation matrix (3, 3).
    Returns:
        torch.Tensor: SH degree-2 rotation matrix (5, 5).
    """
...
def get_sh3_torch(sh1, sh2):
    """
    Compute the SH degree-3 rotation matrix.
    Args:
        sh1 (torch.Tensor): SH degree-1 rotation matrix (3, 3).
        sh2 (torch.Tensor): SH degree-2 rotation matrix (5, 5).
    Returns:
        torch.Tensor: SH degree-3 rotation matrix (7, 7).
    """
...
def get_sh4_torch(sh1, sh2, sh3):
    """
    Compute the SH degree-4 rotation matrix.
    Args:
        sh1 (torch.Tensor): SH degree-1 rotation matrix (3, 3).
        sh2 (torch.Tensor): SH degree-2 rotation matrix (5, 5).
        sh3 (torch.Tensor): SH degree-3 rotation matrix (7, 7).
    Returns:
        torch.Tensor: SH degree-4 rotation matrix (9, 9).
    """
...
class SHRotatorTorch:
    def __init__(self, R, deg=3):
        """
        Initialize the SH rotator using PyTorch.
        Args:
            R (torch.Tensor): Rotation matrix (3, 3). Must be a PyTorch tensor.
            deg (int): Maximum SH degree to compute (up to 4).
        """
    ...
    def __call__(self, shs_in):
        """
        Apply SH rotation to input SH coefficients.
        Args:
            shs_in (torch.Tensor): Tensor containing SH coefficients.
                                   Shape can be (num_coeffs) or (batch_size, num_coeffs).
                                   num_coeffs must correspond to degree `deg`.
                                   Example: deg=0 -> 1, deg=1 -> 4, deg=2 -> 9, deg=3 -> 16, deg=4 -> 25.
        Returns:
            torch.Tensor: Tensor containing rotated SH coefficients, same shape as shs_in.
        """
