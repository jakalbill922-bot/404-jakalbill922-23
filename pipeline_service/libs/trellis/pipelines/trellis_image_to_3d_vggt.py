...
class TrellisImageTo3DPipeline(Pipeline):
    ...
    def preprocess_image(self, input: Image.Image, resolution=518, no_background=True, recenter=True) -> Image.Image:
        ...
        if no_background:
            # Keep RGB colors, just mask with alpha (softer premultiply to preserve colors)
            alpha_mask = output[:, :, 3:4]
            output = np.dstack((
                output[:, :, :3] * alpha_mask,  # Soft premultiply to preserve colors
